...
class DB
{
    ...
    {
        self::$dbh = new PDO("mysql:host=localhost;dbname=php_corse", 'root', '');
    ...
    public function piganate($perPage)
    {
...
$user = DB::raw("select * from users")->piganate(2);
print_r($user);
...
// $users = DB::table("users")->piganate(3);
// echo '<pre>';
// print_r($users);
...
// $users = DB::table("users")->where("name", "%y%", 'like')->orWhere("age", 16)->orderBy("name", "asc")->getAll();
